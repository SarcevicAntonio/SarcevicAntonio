...
<div class="flex flex-col flex-nowrap justify-center xl:flex-row xl:flex-wrap h-feed">
  ...
  <div
    in:fly={{ x: -25, duration: 300, delay: 500 }}
    out:fly={{ x: -25, duration: 300 }}
    class="z-10 flex-1 w-full max-w-screen-md xl:order-last mx-auto xl:ml-0 xl:mr-8 xl:max-w-md">
    {#if allTags && Object.keys(allTags).length > 0}
      <div
        class="flex xl:flex-wrap gap-2 overflow-x-auto xl:overflow-x-hidden overflow-y-hidden max-h-24 my-auto xl:max-h-fit max-w-fit xl:max-w-full pl-8 md:px-0 xl:pl-8">
        {#each allTags as tag}
          <button
            id={tag}
